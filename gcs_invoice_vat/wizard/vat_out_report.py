...
class account_move_line_report(models.TransientModel):
    ...
    def _print_exp_report(self, data):
        res = {}
        import base64
        filename = 'Vat Out Report.xls'
        workbook = xlwt.Workbook(encoding="UTF-8")
        worksheet = workbook.add_sheet('Vat Out Report')
        ...
        fp = io.BytesIO()
        workbook.save(fp)
        export_id = self.env['download.accountml.wizard.report'].create(
        ...
        return {
            'view_mode': 'form',
            'res_id': export_id.id,
            'res_model': 'download.accountml.wizard.report',
            'view_type': 'form',
...
class DownloadAccountMoveLineDownload(models.TransientModel):
    _name = "download.accountml.wizard.report"
    _description = "download.accountml.wizard.report"
